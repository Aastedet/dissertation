...
The following sections describe the algorithms behind the two diabetes classifiers. Overall design and flow of populations are summarised in @fig-classifier-diffs. Pros and cons of their design are discussed in [Chapter -@sec-discussion-of-methods].

::: {#fig-classifier-diffs}
[![](figures/classifierdiffs)](https://aastedet.github.io/dissertation/figures/classifierdiffs.svg)

Overall classifier design and population flow in the Register for selected Chronic Diseases (RSCD) and the Open-Source Diabetes Classifier (OSDC). Click to enlarge.
:::
...
RSCD was implemented  as described below, based on available documentation,[@ruks1doc] which may not completely reflect the source code use by the Danish Health Data Authority to generate the register population. Although the implementation is generally ad verbatim to the documentation, we deviated slightly from documentation where we assessed it to be inaccurate, e.g. having no tie-breaker for classifying individuals with an equal number of T1D and T2D diagnoses who also had a diagnosis of T1D and T2D on the same day as their latest diagnoses. Following documentation to the letter would omit these patients completely from the diabetes population, while our implementation included them as T2D cases.
...
Flow of individuals into the *Health in Central Denmark* survey and the study population of study I. Click to enlarge.
:::
...
T2D prevalence was studied in the above population. In order to study T2D care where it was clinically relevant, the eleven analyses of T2D care were performed in a distinct subsets of the population with T2D, as outlined below and in @fig-study-2-flow:
...
::: {#fig-study-2-flow}
[![](figures/study2flow.png)](https://aastedet.github.io/dissertation/figures/study2flow.png)

Study population flow into each analysis of study II. Click to enlarge.
:::

#### Study III

Register data was used to identify 275,525 individuals with prevalent T2D on 1 January 2019 (index date) from a background population of 4,084,564 individuals above age 25. Among individuals with T2D, 269,011 native Danes or first-generation migrants from the selected origin categories residing at least 3 years in Denmark on the index date were included. After exclusion of 15,647 individuals without records of HbA1c or LDL-cholesterol within three years prior to the index date, the study population contained 253,364 individuals.
...
T2D monitoring was evaluated in time-frames prior to index date matching guideline-recommendations, with three months of leeway added (e.g. monitoring within 15 months is considered guideline-fulfilling, even if the specific type of monitoring is recommended yearly in guidelines):

2.  Monitoring of HbA1c was identified by records of biomarker samples between October 1, 2016 and December 31, 2017 in the *Register of Laboratory Results for Research* or the *Clinical Laboratory Information System*. Additional monitoring of HbA1c using point-of-care tests was identified by records in the *National Health Service Register*.
3.  Monitoring of LDL-C: was identified by records of biomarker samples between October 1, 2016 and December 31, 2017 in the *Register of Laboratory Results for Research* or the *Clinical Laboratory Information System*.
4.  UACR-screening for diabetic nephropathy were identified by records of biomarker samples between October 1, 2016 and December 31, 2017 in the *Register of Laboratory Results for Research* or the *Clinical Laboratory Information System*.
...
6.  Screening for diabetic foot disease was identified by records of diabetes-specific podiatrist services received between October 1, 2016 and December 31, 2017 in the *National Health Service Register.*
...
7.  The outcome measure for HbA1c control was a level ≥53 mmol/mol.
8.  For LDL-C, the outcome measure was a level ≥2.6 mmol/L.
...
In addition, thiazolidinediones (ATC codes A10BG) and repaglinide (ATC codes A10BX02) counted towards GLD combination therapy, but were not analysed as individual drug types due to the very low prevalence of use of these drugs.

### Primary variable of interest: migrant status

Migrant status and country of origin was defined according to the definitions by Statistics Denmark. In order to accomodate the expected heterogeneity between different countries of origin, aggregation of countries was limited to countries with similar reasons for migration, and racial and cultural similarities. Based on the national origins with the most T2D cases and the United Nations M49 geoscheme, the population was grouped into nine origin categories:
...
: Validation concordance table and metrics.
:::
